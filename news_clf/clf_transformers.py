...
def _load_with_local_fallback(loader, description: str, prefer_local: bool = True):
    """Attempt to load a Hugging Face resource from the local cache first.

    Args:
        loader: Callable accepting a single ``local_files_only`` bool argument and
            returning the loaded resource.
        description: Human readable description used in log messages.
        prefer_local: Whether to try ``local_files_only=True`` before falling back to
            a remote download.

    Returns:
        The object returned by ``loader``.

    Raises:
        RuntimeError: If both the local and remote attempts fail.
    """
    ...
    if prefer_local:
        ...
        except Exception as err:  # pragma: no cover - log for visibility, reattempt below
            local_error = err
            logger.warning(
                "Local load for %s failed (%s). Retrying with remote download.",
                description,
                err,
            )
    ...
    try:
        ...
        if local_error is not None:
            raise RuntimeError(
                f"Unable to load {description} from local cache ({local_error}) "
                f"or remote hub ({err})."
            ) from err
        raise
...
class PretrainedModelForOrdinalSequenceClassification(nn.Module):
    def __init__(
        ...
        def _model_loader(local_files_only: bool):
        ...
        self.model = _load_with_local_fallback(
            _model_loader, f"{model_id} backbone", prefer_local=prefer_local
        )

        def _tokenizer_loader(local_files_only: bool):
            return AutoTokenizer.from_pretrained(
                model_id, use_fast=False, local_files_only=local_files_only
            )

        self.tokenizer = _load_with_local_fallback(
            _tokenizer_loader, f"{model_id} tokenizer", prefer_local=prefer_local
        )
...
class PretrainedModelForUnorderedSequenceClassification(nn.Module):
    ...
    def __init__(
        ...
        def _clf_loader(local_files_only: bool):
        ...
        clf_model = _load_with_local_fallback(
            _clf_loader, f"{model_id} sequence classifier", prefer_local=prefer_local
        )
    ...
    def gradient_checkpointing_enable(self, *args, **kwargs):
        return self.deberta.gradient_checkpointing_enable(*args, **kwargs)
    def forward(self, input_ids=None, attention_mask=None, labels=None, **kwargs):
